...
function SecurityClientHandler(options, tokens) {
  ...
  this.options.responseKeyInfoProvider =
    this.options.responseKeyInfoProvider || null
  this.options.validateResponseSignature =
    this.options.validateResponseSignature || false
...
SecurityClientHandler.prototype.AddTimestamp = function(doc, security) {
  var timestamp = utils.appendElement(
      doc,
      security,
      consts.security_utility_ns,
      'u:Timestamp',
      null
    ),
  ...
  var created = utils.appendElement(
      doc,
      timestamp,
      consts.security_utility_ns,
      'u:Created',
      dateFormat(created_time, 'isoUtcDateTime')
    ),
    expires = utils.appendElement(
      doc,
      timestamp,
      consts.security_utility_ns,
      'u:Expires',
      dateFormat(expires_time, 'isoUtcDateTime')
    )
}
...
function X509BinarySecurityToken(options) {
  ...
  if (!this.options.key) {
    throw new Error(
      'Cannot create an X509Token token without specifying a key in the options'
    )
  }
}
...
X509BinarySecurityToken.prototype.extractBase64Key = function(key) {
  if (key.indexOf(BEGIN_CERT) === -1 || key.indexOf(END_CERT) === -1) {
    throw new Error(
      'Provided PEM-format key does not contain certificate information! Try appending the certificate to the end of the file.'
    )
  }
