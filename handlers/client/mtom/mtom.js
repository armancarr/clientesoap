var select = require('../../../xpath').SelectNodes,
  ...
  utils = require('../../../utils'),
  writer = require('./mime-writer'),
  reader = require('./mime-reader')
require('bufferjs')
...
MtomClientHandler.prototype.send = function(ctx, callback) {
  var self = this
  var boundary = 'my_unique_boundary'
  var parts = [
    {
      id: 'part0',
      contentType:
        'application/xop+xml;charset=utf-8;type="' + ctx.contentType + '"',
      encoding: '8bit',
    },
  ]
  ...
  for (var i in ctx.base64Elements) {
    ...
    var binary = new Buffer.from(file.content, 'base64')
  ...
  parts[0].body = new Buffer.from(doc.toString())
  ctx.contentType =
    'multipart/related; type="application/xop+xml";start="<part0>";boundary="' +
    boundary +
    '";start-info="' +
    ctx.contentType +
    '"; action="' +
